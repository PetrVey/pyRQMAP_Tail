# -*- coding: utf-8 -*-
"""
Weibull tail test — numba-optimised version.

Original authors:
  Yaniv Goldschmidt (yaniv.goldschmidt@unipd.it)
  Petr Vohnicky (petr.vohnicky@unipd.it)

Based on the test described in:
...
Original MATLAB code: https://zenodo.org/records/7234708

Differences from the pyTENAX reference version:
  - `weibull_transform` and `sort_rows_numba` are implemented as numba JIT
    (parallel) kernels, replacing the equivalent numpy operations inside
    `create_synthetic_records`. This gives a significant speedup for large
...
def sort_rows_numba(matrix):
    for i in nb.prange(matrix.shape[0]):
        matrix[i].sort()
    return matrix

@nb.njit(parallel=True)
def weibull_transform(random_array, shape, scale):
    n = random_array.size
    result = np.empty(n, dtype=np.float64)
    for i in nb.prange(n):
        result[i] = scale * (-np.log(1.0 - random_array[i]))**(1.0 / shape)
    return result
...
def create_synthetic_records(seed_random: int,
    ...
    random_ordinary_events = weibull_transform(random_array, shape, scale) #changed to numba
    #old
    ...
    records_matrix = sort_rows_numba(records_matrix) #changed to numba
